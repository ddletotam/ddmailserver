...
impl SessionPool {
    ...
    pub async fn start_idle(&self, notifier: Notifier, creds: Credentials) {
...
async fn idle_loop(notifier: Notifier, creds: Credentials) {
    loop {
        notifier(EngineEvent::ConnectionState { state: "connecting".into(), message: None });

        let result = if creds.use_tls {
            run_idle_tls(&notifier, &creds).await
        } else {
            run_idle_plain(&notifier, &creds).await
        };
...
{
    notifier(EngineEvent::ConnectionState { state: "connected".into(), message: None });
