...
func (db *DB) CreateOutboxAttachment(att *models.OutboxAttachment) error {
	...
	query := `INSERT INTO outbox_attachments (outbox_message_id, filename, content_type, size, data, created_at)
		VALUES ($1, $2, $3, $4, $5, $6) RETURNING id`
	err := db.QueryRow(query, att.OutboxMessageID, att.Filename, att.ContentType, att.Size, att.Data, att.CreatedAt).Scan(&att.ID)
	if err != nil {
...
func (db *DB) GetOutboxAttachmentsByMessageID(outboxMsgID int64) ([]*models.OutboxAttachment, error) {
	query := `SELECT id, outbox_message_id, filename, content_type, size, data, created_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&a.ID, &a.OutboxMessageID, &a.Filename, &a.ContentType, &a.Size, &a.Data, &a.CreatedAt); err != nil {
